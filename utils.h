...
const int N = 10;   //Number of columns for a piece to cover on the screen (not the entire screen) = 10 
					//modify colFilled and rowFilled function if you modify N 
...
bool anamoly(int uniteractiveRows){
    for (int i=0;i<4;i++)
        if (/*point_1[i][0]<0 || point_1[i][0]>=N ||*/ point_1[i][1]>=(M-uniteractiveRows))
            return 0;
...
//
std::string name[10] = {"a","b","C","D","e"};
int highScore[10] = {0,0,0,0,0};
